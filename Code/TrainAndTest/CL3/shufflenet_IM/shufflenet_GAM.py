...
class ShuffleNet(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes=4, groups=3):
        ...
        self.gam2 = GAM_Attention(out_channels[1])
        self.gam3 = GAM_Attention(out_channels[2])
        self.gam4 = GAM_Attention(out_channels[3])
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.stage2(x)
        x = self.gam2(x)  # ECA block after stage 2
        x = self.stage3(x)
        x = self.gam3(x)  # ECA block after stage 3
        x = self.stage4(x)
        x = self.gam4(x)  # ECA block after stage 4
        x = self.avg(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
