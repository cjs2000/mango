...
class ShuffleNet(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes=4, groups=3):
        ...
        self.se2 = SEAttention(out_channels[1])
        self.se3 = SEAttention(out_channels[2])
        self.se4 = SEAttention(out_channels[3])
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.stage2(x)
        x = self.se2(x)  # ECA block after stage 2
        x = self.stage3(x)
        x = self.se3(x)  # ECA block after stage 3
        x = self.stage4(x)
        x = self.se4(x)  # ECA block after stage 4
        x = self.avg(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
